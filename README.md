...
| 主界面 | 执行追踪（缓存命中） | 执行追踪（冷检索） | 评测基线 |
|---|---|---|---|
| ![主界面](docs/portfolio/screenshot_1_main_interface.png) | ![执行追踪](docs/portfolio/screenshot_2_execution_trace.png) | ![冷检索执行追踪](docs/portfolio/screenshot_4_cold_execution_trace.png) | ![评测基线](docs/portfolio/screenshot_3_eval_baseline.png) |

演示问题：`GB/T 16252—2023 的名称和适用范围是什么？`

- 主界面：问题、最终回答（含 `[L1]`/`[L2]` 引用）、右侧参考来源清单（每条带 `direct`/`indirect` 证据状态）
- 执行追踪（缓存命中）：同一道题**检索只用 0.2 秒**——因为同进程里已经问过
- 执行追踪（冷检索）：同一道题**最慢节点 `retrieve` 208.5 秒**，仍能交付 2 条 `direct` 证据。
  两张并排看，就是「耗时差三个数量级」的现场（原因见下）
- 评测基线：R 策略 10 题指标 + 已知边界 + 检索诊断（`logs/portfolio_baseline/`）

录屏有两个，**分别是这道题的两种真实耗时，不要混用**：

| 文件 | 时长 | 是什么 |
|---|---:|---|
| `docs/portfolio/demo_recording.mp4` | 35 s | **缓存命中**的那一次（同进程里已经问过这道题）|
| `docs/portfolio/demo_recording_cold.mp4` | 4 分 11 秒 | **冷检索**的那一次（重启应用后第一次问）|

两个都是 H.264、真实时间**未加速未剪辑**、**静音**——按 `作品集Demo讲稿.md` 自己配旁白。
`.webm` 原件在同目录。

截图与录屏都由 `python scripts/capture_portfolio_assets.py` 驱动真实界面产生，
不是手工摆拍；每次运行的耗时与交付判定都留在 `docs/portfolio/` 的
`capture_summary.json`（快版）与 `evidence_cold_run_20260925_150657.json`（冷版）里。

#### ⚠️ 这道题的耗时差三个数量级——根因是**进程内检索缓存**
...
差异全部由 `retrieval_profile.cache_hit` 解释
（`src/retriever.py` 的进程内 `_RETRIEVAL_CACHE`，命中即克隆返回、不跑 dense/rerank）。
下表可直接复现：`python scripts/dump_retrieval_profiles.py`
（原始数据 `docs/portfolio/retrieval_cache_evidence.json`）。
时刻取 checkpoint 落盘时间，即**该次运行的结束时刻**：

| 运行结束 | `cache_hit` | 检索 | 重排 | BM25 | 向量 | 结果 |
|---|---:|---:|---:|---:|---:|---|
| 13:47:41 | **true** | 0.215 s | 136.451 s\* | 17.450 s\* | 2.598 s\* | 可交付，2 条 `direct` |
| 14:00:18 | **false** | **300.047 s** | — | — | — | **超时降级拒答**，0 条来源 |
| 14:10:29 | true | 0.207 s | 179.995 s\* | 22.391 s\* | 2.890 s\* | 可交付，2 条 `direct` |
| 14:15:26 | true | 0.163 s | 179.995 s\* | 22.391 s\* | 2.890 s\* | 可交付 |
| 14:17:32 | true | 0.070 s | 179.995 s\* | 22.391 s\* | 2.890 s\* | 可交付 |
| 14:19:31 | true | 0.068 s | 179.995 s\* | 22.391 s\* | 2.890 s\* | 可交付（仓库截图即此次）|
| 14:49:32 | **false** | **300.012 s** | — | — | — | **超时降级拒答**（该次与一个测试进程并发）|
| 15:10:58 | **false** | **208.491 s** | 177.281 s | 10.780 s | 1.779 s | **可交付，2 条 `direct`**（冷检索录屏即此次）|

\* **定位这条的线索**：缓存命中时 `retrieval_profile` 保留的是**填充缓存那次冷检索**的
组件耗时——五次命中全都带着同一组 `重排 179.995 s / bm25 22.391 s`，
说明它们是同一个缓存条目的五次命中，而不是五次独立检索。

所以这道题的两种耗时是：

- **冷检索**（该进程里第一次问这道题）：**实测 208.5 秒可以完成**；但**三次冷检索里
  有两次撞上 300 秒上限** → `retrieval_timeout` → 降级拒答
...
- 两次降级的原始记录都保留着（`docs/portfolio/evidence_degraded_run_20260925_135405.json`
  与 `docs/portfolio/evidence_cold_timeout_20260925_144324.json`）**不隐藏失败的那几次**
