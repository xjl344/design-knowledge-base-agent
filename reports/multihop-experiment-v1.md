...
#### 补充：压缩 vs 不压缩（同条数）——**结论不成立，判为未决**

又跑了一次探针，两臂同为 `max_evidence=99`，只差是否压缩到 300 字符：

| 臂 | 上下文 | n | mean | median |
|---|---|---|---|---|
| full | 5423~17702 字符 | 18 | 38.14s | 33.20s |
| compress300 | 3609~8338 字符 | 17 | **51.67s** | 45.74s |

配对差值 mean **+13.98s**（compress 更慢）、10 对更慢 / 7 对更快。

**但这个结果站不住，理由有三：**

1. **10/7 不显著**（二项检验 p≈0.31）。
2. **差值几乎全部来自第 2 轮**：逐轮均值 full/compress 为
   32.5/41.0、38.0/**73.7**、43.9/38.0。**去掉第 2 轮后配对均值只有 +2.1s（≈0）。**
3. **答案长度解释不了它**：两轮同配置运行的答案长度相当（532 vs 554 字符），
   所以不是「压缩导致输出变长」。

**并且这暴露了探针自身的一个设计缺陷**：原先按「偶数轮 A,B / 奇数轮 B,A」
**确定性交替**，而周期性交替会与 provider 的周期性慢相**发生混叠**——
某一臂可能每次都撞上慢相。第 2 轮「compress 在同一轮内一致偏慢」
正是这种形态，与真实效应无法区分。

**已修正**：臂顺序改为按 (轮, 题) **随机化**（种子固定以便复现）。
本次结果是在修正前采集的，故**不作为证据**。

**最终结论：上下文大小与压缩在本系统上都没有可测的延迟效应；
延迟由 provider 方差主导。**
...
5. **压缩不损精度**（§4.3 逐题零差异，§4.5 预检独立复核），
   但**其延迟收益未证实**：同条数的压缩/不压缩对照**判为未决**
   （10/7 不显著、差值几乎全来自单轮、且该轮采集于探针顺序修正之前）。
   压缩目前是一个**安全但收益未证实**的选项。
6. **会随分母移动的指标不能当闸门**（已降为只报不卡的诊断项）。
7. **块执行的延迟对比不可信**：必须用交替 A/B，且顺序要**随机化**而非周期性交替
   ——周期性交替会与 provider 的周期性慢相混叠，把噪声做成假效应。
8. **本系统的延迟由 provider 方差主导**，上下文大小与压缩都没有可测效应。
