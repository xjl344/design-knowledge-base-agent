...
**而按实测值设定后，压缩是免费的**：单跳集在 600 下**逐题零差异**、
指标全为 1.000，延迟仍从 40s 降到 **23s（−43%）**。

> 也就是说：300 对多跳集绰绰有余，对单跳集会切掉事实；
> 600 对单跳集完全安全。**同一个动作，代价取决于预算是否贴合事实深度。**
...
5. **字符预算要按题集实测的事实深度来定**；定对了，压缩是免费的（单跳集 −43% 延迟、零精度损失）。
6. **会随分母移动的指标不能当闸门**（`citation_id_usage_ratio` 已改为只报不卡）。

### 推荐的配置

| 题集 | `max_evidence` | `max_chars_per_item` | 依据 |
|---|---|---|---|
| 多跳 6 题 | 全部（99）| **300** | 实测事实最远 150 字符；300 下零跳丢失 |
| 单跳 12 题 | 5 | **600** | 实测事实最远 536 字符；600 下逐题零差异，延迟 −43% |

**新增题集时先测事实深度，再定预算**——不要照搬别的题集的数字。
