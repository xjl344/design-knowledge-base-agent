...
|---|---|---|---|
| 多跳 6 题 | **全部（99）** | 300 | 覆盖率 0.472→0.778；压缩零精度损失 |
| 单跳 12 题 | 5 | 600（现为默认）| 实测事实最深 536 字符；600 下逐题零差异 |

**有实测支撑的是 `max_evidence` 用全部**——覆盖率翻倍且延迟无可测代价。

`max_chars_per_item` 的定位要如实说明：**它只被证明「不损精度」，没有被证明「省时间」**
（实测反而略慢）。保留它的理由是**减少 prompt token（成本）**，
而成本本轮没有测量。若不需要控成本，**不截断是更简单的选择**。
